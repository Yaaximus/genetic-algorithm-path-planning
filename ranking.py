...
def ranking(chr_pop_fitness, new_pop, pop_max):
    ...
    chromo_rank = _ranking_based_on_roulet_wheel_selection(pop_max=pop_max, chr_cum_prob=chromo_cum_prob)

    chromo_pop_ranked = _generate_mating_pool(pop_max=pop_max, chr_rank=chromo_rank, pop=new_pop)
...
def _generate_mating_pool(pop_max, chr_rank, pop):
    ...
    for i in range(pop_max):
        for j in range(int(chr_rank[i, 0])):
            temp[i] = np.append(ranked_chr, pop[i], axis=0)
